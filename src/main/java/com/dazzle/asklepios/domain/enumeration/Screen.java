package com.dazzle.asklepios.domain.enumeration;

import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonValue;

public enum Screen {
    //Waseel Integration
    WASEEL_ELIGIBILITY_REQUESTS,
    WASEEL_PRE_AUTHORIZATION_REQUESTS,

    // Scheduling
    SCHEDULING_SCREEN,
    AVAILABILITY_TEMPLATES_NEW,
    APPLY_TEMPLATE,
    APPOINTMENT,

    // Inventory Management
    INVENTORY_TRANSFER,
    INVENTORY_TRANSACTIONS,
    INVENTORY_TRANSFER_APPROVAL,
    PRODUCTS_CATALOG,
    WAREHOUSE_SETUP,
    WAREHOUSE_ITEMS_SETUP,
    DEPARTMENT_STOCK,

    // Billing & Finance
    LEDGER_ACCOUNT,
    PRICE_LISTS,
    CLAIMS,
    PATIENTS_LIST,

    // System Setup
    ALLERGENS,
    POTENTIAL_DUPLICATE,
    BRAND_MEDICATIONS,
    LISTS_OF_VALUE,
    DATA_VALIDATION_MANAGER,
    DVM,
    FORM_TEMPLATE,
    ACCESS_ROLES,
    FACILITIES,
    DEPARTMENTS,
    UOM_GROUP,
    PRESCRIPTION_INSTRUCTIONS,
    AGE_GROUP,
    PRACTITIONERS,
    ACTIVE_INGREDIENTS,
    METADATA,
    RESOURCES,
    DENTAL_ACTIONS,
    USERS,
    CATALOG_SETUP,
    DIAGNOSTICS_SETUP,
    VACCINE_SETUP,
    PROCEDURE_SETUP,
    ROOM_BED_SETUP,
    MED_MATRIX_SETUP,
    SERVICES_SETUP,
    VACCINE_SCHEDULE_SETUP,
    CHECKLISTS,
    OPERATION_SETUP,
    PURCHASE_APPROVAL,
    QUESTIONNAIRE_SETUP,
    MEDICATION_MATRIX_SETUP,
    MEDICATION_SCHEDULE_SETUP,
    VISIT_DURATION,
    CLINICAL_PROTOCOLS_SETUP,
    SHIFT_SETUP,
    SUPPLIER_SETUP,
    SURGICAL_KITS_SETUP,
    USER,
    TEST_REPORT_TEMPLATE_SETUP,
    PRODUCTS_SETUP,
    LANGUAGE_SETUP,
    COUNTRY_SETUP,
    PAYER,
    POLICY_DEFINITION,
    NPHIES_PAYER,
    // Front Desk Office
    QUICK_APPOINTMENT,
    PATIENT_REGISTRATION,
    FACILITY_PATIENTS,
    INFORMATION_DESK,
    FILES_MERGE,
    ENCOUNTERS_LIST,
    // Emergency
    ER_TRIAGE,

    ER_DEPARTMENT,
    URGENT_CARE_DEPARTMENT,
    ER_DASHBOARD,

    // Purchasing
    SUPPLIER_MANAGEMENT,
    QUOTATION_MANAGEMENT,
    PAYMENT_TRACKING,
    LIST_OF_REQUISITION,
    PURCHASE_RETURN,
    PURCHASE_REQUISITION,
    REQUISITION_APPROVAL,
    PURCHASE_ORDERS,
    GOODS_RECEIPT_NOTE,

    // Telemedicine
    TELE_CONSULTATIONS_REQUESTS,

    // Inpatient Care
    INPATIENT_LIST,
    WAITING_LIST,


    // Day Case
    DAY_CASE_PATIENT_LIST,

    // Procedures
    PROCEDURE_REQUESTS_LIST,

    // Operation Theater
    OPERATION_REQUESTS,
    RECOVERY_ROOM,
    OPERATION_ROOM_MATERIALS,

    //EMR Module

    ELECTRONIC_MEDICAL_RECORDS,

    // Clinics Management
    PATIENTS_VISITS_LIST,
    REVIEW_RESULTS,
    TESTS_RESULTS,


    // Laboratory
    CLINICAL_LABORATORY,

    // Radiology
    IMAGING_RADIOLOGY,

    // Pharmacy
    INTERNAL_DRUG_ORDERS,
    EPRESCRIPTIONS,
    CONTROLLED_MEDICATIONS,

    // Coding Module
    CPT,
    ICD_10,
    LOINC,
    CDT_CODES,
    SNOMED_CT,



    // System Configurations
    ORGANIZATION_DEFINITION,
    GENERAL_SETTINGS,
    CONFIGURATIONS,
    ORGANIZATION_HOLIDAY,
    URGENT_CARE_TRIAGE,
    WASEEL_SBS_SETUP,
    PRICE_LIST_SETUP,
    PRICE_LIST_ITEM_DASHBOARD,
    BILLING_CONFIGURATION,
    TAX_CONFIGURATION,
    DISCOUNT_CONFIGURATION,
    COVERAGE_MANAGEMENT,
    NURSE_ASSESSMENT,
    PATIENT_MERGE_CONFIG,
    SYSTEM_CONFIGURATION_SETUP,
    BILLING_RULE_SETUP,
    FINANCIAL_DOCUMENT_NUMBERING,
    //Notification Module
    NOTIFICATION_RULE,
    EMAIL_NOTIFICATION,
    IN_APP_NOTIFICATION,
    SMS_NOTIFICATION,
    WHATSAPP_NOTIFICATION,
    WASEEL_CLAIMS,
    EMAIL_SETTINGS,
    WHATSAPP_SETTINGS,
    PATIENT_SATISFACTION_SURVEY_RESPONSES,
    POINT_OF_SALE_CHECK_IN,
    PATIENT_ENCOUNTER_REPORT,
    POINT_OF_SALE_CONFIGRATION,
    PUSH_NOTIFICATION,
    DOCUMENT_MANAGEMENT,
    STIMULSOFT_REPORT_DESIGNER,
    CLINICS_MODULE_REPORTS,
    URGENT_CARE_MODULE_REPORTS,
    RADIOLOGY_MODULE_REPORTS,
    LABORATORY_MODULE_REPORTS,
    PATIENT_MODULE_REPORTS,
    PHARMACY_MODULE_REPORTS,
    BILLING_MODULE_REPORTS,
    APPOINTMENT_MODULE_REPORTS,
    POINT_OF_SALE_WEBHOOK_LOGS,
    STIMULSOFT_DASHBOARD_DESIGNER;



    @JsonCreator
    public static Screen fromValue(String value) {
        return Screen.valueOf(value.toUpperCase().replace(" ", "_"));
    }

}
