...
class InventoryTransactionAttachmentsServiceTest {
    ...
    void upload_Success() throws Exception {
        ...
        UploadInventoryTransactionAttachmentVM vm = mock(UploadInventoryTransactionAttachmentVM.class);
        when(vm.file()).thenReturn(file);

        ArgumentCaptor<InventoryTransactionAttachments> cap = ArgumentCaptor.forClass(InventoryTransactionAttachments.class);
        when(repo.save(cap.capture())).thenAnswer(inv -> inv.getArgument(0));

        var result = service.upload(55L, vm);

        InventoryTransactionAttachments saved = cap.getValue();
        ...
        verify(storage).put(
                startsWith("inventoryTransactions/55/"),
                eq("image/png"),
                eq(1024L),
    ...
    void list_ReturnsFromRepo() {
        when(repo.findByTransactionIdAndDeletedAtIsNullOrderByCreatedDateDesc(77L))
                .thenReturn(List.of(new InventoryTransactionAttachments()));
    ...
    void softDelete_SetsDeletedAtOnce() {
        InventoryTransactionAttachments entity = new InventoryTransactionAttachments();
        entity.setId(5L);
